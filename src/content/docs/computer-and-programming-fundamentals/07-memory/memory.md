...
The stack is a region of memory used for the management of function calls and local variables. It operates on the principle of [stack data structure](/computer-science-notes/data-structures-and-algorithms/stack). Each time a function is called, a new stack frame is created and pushed onto the stack. The stack frame contains information such as function parameters, return address, and local variables.
...
When we assign `a = 5`, a memory object representing the value `5` is created and stored. The variable `a` is then associated with this object. Later, when we assign `a = 3`, assuming numbers are immutable, a new memory object representing the value `3` is created. The variable `a` is updated to reference this new object. The original object representing `5` remains in memory until it is cleaned up by the [garbage collector](/computer-science-notes/computer-and-programming-fundamentals/memory#garbage-collection) or manually released in lower-level programming languages.

#### Object

Object can refer to object in [OOP](/computer-science-notes/computer-and-programming-fundamentals/object-oriented-programming), which is a particular instance of a class. In general, when we refer to object, we tend to describe it as anything that has name or identifier, such as a variable.
...
Pointer which stores memory address allows us to have direct memory access. This is useful for implementing data structures like [tree](/computer-science-notes/data-structures-and-algorithms/tree) or [linked list](/computer-science-notes/data-structures-and-algorithms/linked-list), where each node need to connection with other node. We can allow connection between node by having a pointer that points to other node's address.
...
In this C code, an array of integer called `arr` is created. Then, we create a pointer called `p` that points to `arr`. This works, because essentially [array is just a pointer under the hood](/computer-science-notes/data-structures-and-algorithms/array) that points to the first element of the array. So, `arr` and `p` both points to the address of element 1.
