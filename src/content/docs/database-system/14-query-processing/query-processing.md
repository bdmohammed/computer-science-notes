...
**Index-based algorithms** is a set of algorithms that leverage [database indexes](/computer-science-notes/database-system/database-index), such as selection and join. In some cases, clustered indexes may be faster than non-cluster index, because the physical ordering of data in disk is already known.
