...
**Constraints** are rules or conditions that are enforced on the data in a database table. In [SQL](/computer-science-notes/database-system/query-language#sql), there are several types of constraints that can be applied to database tables:
...
- `CREATE INDEX`: Creates [index](/computer-science-notes/database-system/database-index) in table for performance optimization.
