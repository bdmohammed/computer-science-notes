...
**NoSQL (or non-relational database)** is a type of databases that doesn't follow the relational model principles. Unlike [relational databases](/computer-science-notes/database-system/relational-data), which store data in tables with predefined schemas, NoSQL databases is more flexible in approach to data storage and management. NoSQL typically relies on a specific data structure to handle large volumes of data, including unstructured and semi-structured data.
...
One way to implement key-value database is to use [hash table](/computer-science-notes/data-structures-and-algorithms/hash-table). Hash table is a data structure that stores data in an [array](/computer-science-notes/data-structures-and-algorithms/array), using a [hash function](/computer-science-notes/computer-security/hash-function) to map keys to array indices. This allows for very fast lookups and insertions, which is suitable for storing random or unstructured data.
...
Document database includes [JSON](/computer-science-notes/digital-media-processing/json), [XML](/computer-science-notes/digital-media-processing/xml), and YAML.
...
- Unlike relational databases, document databases have limited support for [transactions](/computer-science-notes/database-system/transactions). This can make it challenging to maintain data consistency and integrity in certain scenarios.

### Graph

A graph NoSQL database uses a [graph](/computer-science-notes/data-structures-and-algorithms/graph) structures to represent and store data.
...
Under the hood, node is represented as a fixed-size record or object on the disk. An edge relationship is represented as [doubly linked list](/computer-science-notes/data-structures-and-algorithms/linked-list#doubly-linked-list), it has pointer to the start and end nodes. Node's properties are organized in a linked-list, with each node being a key-value pair.
...
NoSQL typically lacks strong consistency and true transactions compared to relational databases. BASE properties are in contrast to [ACID properties](/computer-science-notes/database-system/transactions#acid) in relational database.
