...
Where $\prod$ is a symbol for [projection](/computer-science-notes/database-system/query-language#projection), and $\bowtie$ is a symbol for natural joins.
...
In the book data above, the [composite key](/computer-science-notes/database-system/relational-data#other-keys) is title and format. Partial dependency is when one of the column depend on one of the composite key, but not all. For example, the `Price` column depends on format, but not title.
