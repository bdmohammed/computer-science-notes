...
- **[Logging (computing) — Wikipedia](/computer-science-notes/<https://en.wikipedia.org/wiki/Logging_(computing)>)**
...
- **[Replication (computing) — Wikipedia](/computer-science-notes/<https://en.wikipedia.org/wiki/Replication_(computing)>)**
...
Database operations are grouped into a unit of [transaction](/computer-science-notes/database-system/transactions). Transaction log is the log file that contains all the transactions and changes made to the database over time.
...
Database logs are linked together, forming a [linked list](/computer-science-notes/data-structures-and-algorithms/linked-list). A log record is made up of:
...
- **Master-Slave Replication**: Also known as leader-follower replication, this approach rely on two types of node. A _master_ database handles write operations, while one or more _slave_ databases replicate the data from the master. The slaves are intended to be read-only, they can only serve read queries from clients. The master should be responsible for propagating the changes to the slaves asynchronously or synchronously in some amount of time.
  ...
  The downside of using single master is, it could be overwhelmed by all the write request. Furthermore, it can lead to a single point of failures when the master fails. For that reason, when a master fails, it may be needed to select a new master through algorithm like [leader election](/computer-science-notes/cloud-computing-and-distributed-systems/distributed-systems-model#leader-election).
...
See [RAID](/computer-science-notes/operating-system/disk-management#raid).
