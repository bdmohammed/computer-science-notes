...
- Choosing which algorithm should we use to implement the operator, for example, deciding whether a [nested-loop join](/computer-science-notes/database-system/query-processing#nested-loop-join) or a [hash-join](/computer-science-notes/database-system/query-processing#hashing-based) should be used.
...
- **[Top-down approach](/computer-science-notes/data-structures-and-algorithms/dynamic-programming#top-down)**: Start from the root of the logical query plan down to bottom of the tree. For each possible implementation, we enumerate the physical plans and evaluate their costs.
- **[Bottom-up approach](/computer-science-notes/data-structures-and-algorithms/dynamic-programming#bottom-up)**: Keeps track of the plan of the least cost for each subexpression. By considering the cost of each subexpression, the overall cost of the query plan can be obtained by combining them all.
...
- **Physical Plan Implementation**: The specific details regarding access methods for stored relations and algorithms for implementing relational-algebra operators. Access methods refer to the techniques used to retrieve tuples from stored relations, such as [table scans, index scans, or sort scans](/computer-science-notes/database-system/query-processing#physical-query-plan-operators).
