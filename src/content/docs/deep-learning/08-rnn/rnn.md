...
**Recurrent Neural Network (RNN)** is a type of [neural network](/computer-science-notes/deep-learning/neural-network) specifically designed for processing sequential data or data that needs to be processed in specific order such as [NLP](/computer-science-notes/deep-learning/deep-learning-tasks#natural-language-processing-nlp) data.
...
It doesn't have to be the last output, another mechanism called **[attention](/computer-science-notes/deep-learning/transformers/attention-mechanism)** captures the only important or relevant information, which isn't always the last.
...
2. **Decoder initialization**: The decoder can also be a standard RNN model, the context vector serve as the initial hidden state. The decoder however, doesn't take additional input, it simply calculates current hidden state from the previous hidden state. In the formula below, the second term from the encoder's formula is removed.
   ...
   The output multiplies hidden state at time $t$ with some weight and it goes into the [softmax activation function](/computer-science-notes/deep-learning/neural-network#softmax-activation-function).
...
:::note
By standard RNN model, it can also be other type of RNN like [LSTM](/computer-science-notes/deep-learning/lstm) and [GRU](/computer-science-notes/deep-learning/gru).
:::
