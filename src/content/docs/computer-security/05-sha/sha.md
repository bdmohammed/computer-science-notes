...
1. **Preprocessing**: The input message is padded to a multiple of the block size. The padding includes adding a 1-bit followed by a series of 0-bits until the desired length is reached (it should be [congruent](/computer-science-notes/computer-security/math-concepts#congruence) to $−64 ≡ 448 \space (\text{mod } 512)$).
