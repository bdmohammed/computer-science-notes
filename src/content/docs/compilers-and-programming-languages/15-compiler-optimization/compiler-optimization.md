...
- **Control flow**: Targets control flow structure (e.g., loops and conditionals), such as reducing the number of branching to reduce [pipelining stall](/computer-science-notes/computer-organization-and-architecture/cpu-design#pipelining).
...
Constant folding simplify expressions (or part of them) that involve constants, which is known during compile-time. For example, the [second calculation in a day](/computer-science-notes/compilers-and-programming-languages/intermediate-representation#optimization) effectively demonstrate this concept.
...
- `x * 8` is optimized to `x << 3`, as [bit shifting](/computer-science-notes/computer-and-programming-fundamentals/bitwise-operation#bit-shifting) can be faster than multiplication. This is similar to division.
...
In branching, CPU can try to make an educated guess about the outcome of a branch instruction before it is actually executed. This will avoid [pipeline stalls](/computer-science-notes/computer-organization-and-architecture/cpu-design#pipelining), which further improves performance.
...
The detection of dead code is typically performed on [control flow graph](/computer-science-notes/compilers-and-programming-languages/intermediate-representation#control-flow-graph).
...
The above is an image of tree templates of some instructions in [x86](/computer-science-notes/computer-organization-and-architecture/isa#x86).
...
This conflict graph describes a [graph coloring problem](/computer-science-notes/data-structures-and-algorithms/graph), where the nodes represent variables and the edges represent interference between variables. The goal is to assign each node a different color (registers), such that no two adjacent nodes has the same color (avoiding interference).
