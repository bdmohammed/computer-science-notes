...
- High availability, it would be an AP system in the context of [CAP theorem](/computer-science-notes/software-engineering/system-design#cap-theorem).
...
- Meeting client act as the codecs for video/audio streamed by participant. The codec can be something like [AVC](/computer-science-notes/digital-media-processing/mp4#mp4-compression-h264) or SVC.
...
- Simple data such as chat or reaction can be sent to the server using standard protocol such as [HTTPS](/computer-science-notes/computer-networking/http-https). The client will also keep track of these.
- Video stream is sent using [RTP](/computer-science-notes/computer-networking/rtp), specifically the secure RTP.
...
- **Meeting Server**: A server dedicated to handle meeting.
  ...
  - Could use [message broker](/computer-science-notes/backend-system/message-broker) technology like RabbitMQ to handle the distribution of messages and events among meeting server instances.
...
- **Web Server**: We will have another server for serving web-based client. This server will also be used for serving request that aren't meeting related, such as authentication.
  ...
  - Provide [REST API](/computer-science-notes/backend-system/rest-api) services. Similarly, we can use [Ktor](https://ktor.io/) for this. Ktor provide a simple way to create authentication, serialization, and other server stuff.
