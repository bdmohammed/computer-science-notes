...
HTTP operates on top of the [TCP/IP protocol suite](/computer-science-notes/computer-networking/tcp-ip-model), using [Transmission Control Protocol (TCP)](/computer-science-notes/computer-networking/tcp-protocol) as the underlying transport protocol. It typically uses port 80 for communication, although other ports such as 8080 are also commonly used.
...
The protocol follows a client-server model, where the client, typically a [web browser](/computer-science-notes/internet-and-web/web-browser), sends requests to the server, and the server responds with the requested data.

![HTTP client-server mechanism](./client-server-mechanism-http.png)  
Source: https://www.freecodecamp.org/news/what-is-http/

HTTP requests consist of a method that indicates the type of request, a [Uniform Resource Locator (URL)](/computer-science-notes/internet-and-web/web-url) that specifies the resource being requested, and optional headers that provide additional information about the request.
...
![An example of HTTP response from HTTP — Wikipediapage, obtained from the network console in Firefox web browser](./http-response.png)
...
- HTTP Response Format:
  ...
  - **Blank Line**: A blank line indicating the end of the headers section.
  - **Response Body**: Contains the actual content of the response, such as HTML pages, [JSON](/computer-science-notes/digital-media-processing/json), or binary data.
...
3. **Key Exchange**: In the next step, there will be an [asymmetric key exchange](/computer-science-notes/computer-security/encryption#symmetric--asymmetric-encryption) where the client and server exchange keys. This key exchange process includes sending the necessary information required to encrypt the data. This information typically includes the [key exchange algorithm](/computer-science-notes/computer-security/encryption#key-exchange), encryption algorithm, and [hash function](/computer-science-notes/computer-security/hash-function) to be used for the TLS protocol.

4. **Data Transmission**: After the key exchange, a session key is generated. The session key is a [symmetric encryption](/computer-science-notes/computer-security/encryption#symmetric--asymmetric-encryption) key used to encrypt and decrypt the data. The client possesses the public key, while the server has the corresponding private key. In simple terms, the session key can only be used with the public and private keys owned by the client and server. The data is encrypted using the session key, enabling secure transmission between the client and server.
...
:::tip
Find more about general [encryption](/computer-science-notes/computer-security/encryption) and its terminology including [public and private key](/computer-science-notes/computer-security/encryption#public--private-key), [key exchange](/computer-science-notes/computer-security/encryption#key-exchange). Also, about [symmetric and asymmetric encryption](/computer-science-notes/computer-security/encryption#symmetric--asymmetric-encryption).
:::
