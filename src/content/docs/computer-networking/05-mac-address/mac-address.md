...
IP address might change sometimes because of various reason such as configuration change or [DHCP](/computer-science-notes/computer-networking/dhcp). MAC address act as a physical hardware identifier that never changes. As it is assigned permanently by manufacturer, no two devices should have the same MAC address.
