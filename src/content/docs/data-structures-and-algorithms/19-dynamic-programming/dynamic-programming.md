...
The difference is, [divide and conquer](/computer-science-notes/data-structures-and-algorithms/divide-and-conquer) approach divides the problem into non-overlapping subproblems that are solved independently. In other word, each subproblem doesn't depend on each other, we can't apply the solution of a subproblem into another subproblem.
...
In the top-down approach, we will start with the original problem and breaks it down into smaller subproblems. We will then solve these subproblems [recursively](/computer-science-notes/data-structures-and-algorithms/recursion).
...
Memoization is a technique used in dynamic programming, specifically in the top-down approach to improve the algorithm efficiency. Memoization optimize the execution by storing (or **[caching](/computer-science-notes/computer-and-programming-fundamentals/caching)**) the result of function calls for specific input values and reusing them when the same inputs occur again.
...
The memoization technique stores all the result of every Fibonacci's sequence in some data structure that can be random accessed efficiently like [array](/computer-science-notes/data-structures-and-algorithms/array) or [hash map](/computer-science-notes/data-structures-and-algorithms/hash-table).
