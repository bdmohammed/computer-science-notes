...
**PNG (Portable Network Graphics)** is a digital image format that uses [lossless compression](/computer-science-notes/digital-signal-processing/compression#lossless-compression), meaning it stores image without any loss of information, but with bigger file sizes compared to other format like [JPEG](/computer-science-notes/digital-media-processing/jpg-jpeg).

Other characteristic of PNG is that it supports [alpha channel](/computer-science-notes/computer-graphics/computer-images-part-2#alpha) transparency. In other word, PNG can display transparent areas within an image. Alpha channel is stored alongside the RGB channel, which mean PNG format require an extra 8 bits for each color, making a single pixel took 32 bits.
...
2. **DEFLATE**: DEFLATE is a combination of LZ77 and [Huffman encoding](/computer-science-notes/digital-signal-processing/compression#huffman-encoding) algorithm.
