...
GraphQL typically uses [HTTP](/computer-science-notes/computer-networking/http-https#http) as the transport layer, same as [SOAP](/computer-science-notes/backend-system/soap), the request is encapsulated in HTTP POST method. GraphQL responds with HTTP response followed by a [JSON](/computer-science-notes/digital-media-processing/json) data object that contains the data requested by client.
