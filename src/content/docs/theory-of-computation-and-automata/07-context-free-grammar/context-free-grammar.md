...
Context-free grammar is the grammar that generates context-free language, which is recognized by [pushdown automata](/computer-science-notes/theory-of-computation-and-automata/pushdown-automata). The production rule is:

$A \rightarrow \alpha$, where $\alpha = \{V \cup \Sigma^*\}$ and $A \in V$.

Similar to the [general](/computer-science-notes/theory-of-computation-and-automata/formal-grammar#grammar) and [regular grammar](/computer-science-notes/theory-of-computation-and-automata/formal-grammar#regular-grammar), a non-terminal symbol can produce $\alpha$ which can be any symbol from the set of all possible strings of symbols that can be generated using both the non-terminal symbols and the terminal symbols of the grammar, including the empty string.
...
:::info
See also [tree data structure](/computer-science-notes/data-structures-and-algorithms/tree).  
...
Pumping lemma for context-free languages is to **prove that a language is not context-free**. In contrast, [pumping lemma for regular language](/computer-science-notes/theory-of-computation-and-automata/regular-languages-part-2#pumping-lemma-for-regular-languages) proves that a language is not regular.
