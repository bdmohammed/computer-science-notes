...
Regular language is a language that can be recognized by some [finite state machine](/computer-science-notes/theory-of-computation-and-automata/finite-automata). It is constructed by three set operations, union, concatenation, and Kleene star.
...
- **Union ($+$)**: Union operation is like a [logical OR](/computer-science-notes/computer-and-programming-fundamentals/bitwise-operation#or), if we have two regular expression, $R_1, R_2$, union between them would be represented as $R_1 + R_2$. This mean we are making a pattern that match either $R_1$ or $R_2$.
