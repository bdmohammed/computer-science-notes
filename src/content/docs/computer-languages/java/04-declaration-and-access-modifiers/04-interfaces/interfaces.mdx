...
│
├── Members
│   ├── abstract methods      (default — always public abstract)
│   ├── default methods       (Java 8+ — concrete with body)
│   ├── static methods        (Java 8+ — interface-level utility)
│   ├── private methods       (Java 9+ — internal helper)
...
│
├── Special Types
│   ├── Marker interface      (no methods — tag only)
...
└── Rules
    ...
    ├── No instance fields
    ├── No constructors
    └── All methods implicitly public (except private)
```
...
<Code lang="java" title="Basic interface declaration and implementation" code={`
// Declaration
interface Animal {
    void eat();      // implicitly public abstract
    void sleep();    // implicitly public abstract
}
// by default all methods in interface are public abstract and we cannot declare them as private or protected otherwise we will get compile time error because they are meant to be implemented by other classes and if we declare them as private or protected then they will not be accessible to other classes for implementation.

// Variables in interface are by default public static final and we cannot declare them as private or protected otherwise we will get compile time error because they are meant to be accessed by other classes and if we declare them as private or protected then they will not be accessible to other classes for access.

// Interfaces cannot have constructors because they cannot be instantiated. They are meant to be implemented by other classes, and the implementing classes are responsible for providing the implementation of the interface methods. Since interfaces do not have instance variables (only static final constants), there is no need for a constructor to initialize any state.

//hence inside interface 
// void m1();
// public void m1();
// abstract void m1();
// public abstract void m1();
// all this methods are same and equal
...
Interface member implicit modifiers:
  Fields:          public static final  (always — no exceptions)
  Abstract methods: public abstract     (always)
  default methods:  public              (explicit body required)
  static methods:   public              (Java 8+)
  private methods:  private             (Java 9+ — no abstract, no static)
  private static:   private static      (Java 9+)

> 🔑 Interface fields are **always** `public static final` — they are constants, never instance state. You cannot have instance fields in an interface. Attempting `int count = 0` in interface = `public static final int count = 0`.
```
...
Implementation rules:
  ...
  ├── Implementing method must be public (cannot narrow access)
  ├── Can implement multiple interfaces
  ├── Implementing method throws ⊆ interface method throws
  └── @Override recommended — compiler validates
```

### 💻 Syntax Examples

<Code lang="java" title="Valid inheritance combinations" code={`
...
<Code lang="java" title="Implicit modifiers — all declarations are equal" code={`
interface Shape {
...
<Code lang="java" title="default methods enable interface evolution" code={`
interface Vehicle {
...
<Code lang="java" title="static methods belong to interface, not instances" code={`
interface MathUtils {
...
<Code lang="java" title="private methods for default/static method helpers" code={`
interface Logger {
...
<Code lang="java" title="All variable declarations are equivalent" code={`
interface Config {
...
<Code lang="java" title="These modifiers cause compile errors" code={`
interface Config {
...
<Card title="Case 3: Same Variable ⚠️" icon="caution">
    ```java
    interface A {
        int x = 777;
    }
...
<Code lang="java" title="Framework-style marker check" code={`
// Simplified serialization framework
class Serializer {
...
<Code lang="java" title="With adapter — clean and focused" code={`
// Adapter provides empty implementations
...
public interface PaymentProcessor {
    ...
    // default and static methods allowed — don't count as abstract
    default PaymentProcessor withLogging() {
        return request -> {
            log.info("Processing: {}", request.getId());
            PaymentResult result = this.process(request);
            log.info("Processed: {}", result.getTransactionId());
            return result;
        };
    }
}
...
//   ✅ Documents intent
//   ✅ Compiler error if more than one abstract method added
//   ❌ Optional — interface is functional without annotation
//      if it has exactly one abstract method

// Lambda assigns to functional interface
PaymentProcessor processor = request -> {
    // implementation
    return PaymentResult.success("txn-123");
};

// Method reference assigns to functional interface
PaymentProcessor processor2 = stripeService::charge;
```

```
JDK functional interfaces (java.util.function):
...
```java
// Restricts which classes/interfaces can implement
